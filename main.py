...
# Page configuration
st.set_page_config(
    page_title="Foundation Takeoff Assistant",
    page_icon="🏗️",
    layout="wide"
)
...
# Main title
st.title("🏗️ Foundation Takeoff Assistant")
st.markdown("""
Upload foundation plan images to automatically estimate concrete quantities. 
""")
...
if uploaded_files:
    ...
    if not st.session_state.images_analyzed and len(uploaded_files) > 0:
        ...
        with col1:
            if st.button("Generate Foundation Takeoff", type="primary"):
                with st.spinner("Analyzing plans and calculating foundation takeoff..."):
...
st.markdown("### 💬 Foundation Takeoff Analysis Chat")
...
if st.session_state.images_analyzed:
    if prompt := st.chat_input("Ask questions about the foundation takeoff..."):
...
st.markdown("---")
st.markdown("""
### How to Use This Foundation Takeoff Assistant:
1. Upload clear images of your foundation plans (multiple images supported)
2. Click "Generate Foundation Takeoff" to get an automated calculation
3. Review the detailed breakdown of concrete quantities
...
if st.session_state.messages:
    ...
    st.download_button(
        "Download Foundation Takeoff Report",
        chat_history,
        file_name="foundation_takeoff_report.txt",
        mime="text/plain"
    )